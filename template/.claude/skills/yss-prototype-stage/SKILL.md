...
把产品设计影响收敛为跨 Agent 一致的产物、证据和 Harness Agent 回流合同。它不替代 Harness Orchestrator 的门禁裁决，也不把原型当作生产前端代码。

## 进入条件与主入口

- 先由 Harness Orchestrator 判断产品设计影响；无行为变化的孤立视觉修复记录 `not-applicable`，不创建空资产。
