# Slice Implementation Contract v1

implementation_path_policy: git-submodule-harness-apps

由 yss-router 生成草案，由 harness-orchestrator 校验、持久化和批准。合同以一个垂直切片为边界，四个 Agent 必须消费同一个 contract_id / contract_version。

## 合同结构

slice_contract:
  schema_version: 1
  contract_id: slice-implementation-contract-v1:<slice-id>
  contract_version: 1
  slice_id:
  status: draft
  lifecycle_refs:
...
API schema、数据库 schema、状态机、聚合、不变量、Gateway、写路径、测试 seam 或验证命令发生实质变化时，当前合同变为 stale，所有下游任务暂停并递增 contract_version。
