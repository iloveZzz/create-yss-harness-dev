...
`yss-product-lifecycle` 已退役，不再作为 Router alias、默认发现入口、公开技能或独立物理技能存在。

迁移到：

- 正式编排入口：`harness-orchestrator`
- 领域战术设计：`architecture-agent` 使用 `yss-tactical-design`
- 垂直切片合同：`yss-router` 编译 Slice Implementation Contract 草案，由 Harness Orchestrator 批准
...
`yss-stage-decision` 已退役，不再作为 Router alias、默认发现入口或独立物理技能存在。
