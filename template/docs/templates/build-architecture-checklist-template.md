...
| 原型确认后的后端脚手架顺序：工程基线 → 实现合同编译器 脚手架合同 draft / Harness Orchestrator 批准 → `yss-ddd-scaffold-generator` → `yss-backend-scaffold-parent` → 实现合同编译器 重编译；脚手架不得生成业务行为 | Harness Orchestrator / `yss-ddd-scaffold-generator` |  | `implemented` / `seam-deferred` / `drift` / `violation` / `not-applicable` |  | 顺序缺失、生成器混入业务行为或缺 Execution Result 时阻断 |
...
| 文档语言：持久化生命周期文档、实施记录、审查报告、发布说明和 Git checkpoint 正文必须使用中文，英文 skill / 模板不得原样落地 | `AGENTS.md` / `harness-orchestrator` |  | `implemented` / `seam-deferred` / `drift` / `violation` / `not-applicable` |  | 仅保留必要英文技术标识、命令和 metadata |
