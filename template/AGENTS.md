...
- 无可复用后端工程时，`harness-orchestrator` 在技术设计前根据批准需求与工程约束给出 `domain-driven` / `layered-mvc` 推荐并由用户逐项目确认；确认后分别使用 `yss-ddd-scaffold-generator` / `yss-layered-mvc-scaffold-generator`。前端使用 `yss-frontend-scaffold-generator`。
