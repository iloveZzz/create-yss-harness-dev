...
const DEFAULT_TEMPLATE_REF = "34a1e4380f43e161f8694efb6cb7a47d7ced38ef";
...
function listWorkingTreeFiles(sourceRoot) {
  const files = [];
...
function copyTrackedFiles(sourceRoot, manifest, destinationRoot, trackedFiles) {
  ...
  for (const relativePath of sourceFiles) {
    ...
    for (const trackedTargetPath of sourceFiles) {
...
function isLocalTemplateRepo(repo) {
  const resolved = path.resolve(repo);
  return (
    fs.existsSync(path.join(resolved, "yss-project.yaml")) &&
    fs.existsSync(
      path.join(resolved, ".template-source/distribution/template.manifest.json"),
    )
  );
}

function defaultTemplateRepo() {
  if (process.env.YSS_HARNESS_TEMPLATE_REPO) {
    return process.env.YSS_HARNESS_TEMPLATE_REPO;
  }
  const sibling = path.resolve(packageRoot, "../yss-harness-dev-agent");
  if (isLocalTemplateRepo(sibling)) {
    return sibling;
  }
  return "https://github.com/iloveZzz/yss-harness-dev-agent.git";
}
...
try {
  ...
  const sourceManifestPath = path.join(
    sourceRoot,
    ".template-source/distribution/template.manifest.json",
  );
  if (!fs.existsSync(sourceManifestPath)) {
    throw new Error(`模板源缺少分发清单：${sourceManifestPath}`);
  }
  ...
  const snapshotMetadata = {
    schemaVersion: 1,
    templateName: "yss-harness-dev-agent",
