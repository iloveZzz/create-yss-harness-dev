...
实例写入 `.yss-harness.json`，`profileId` 为 `harness.dev-agent-slice`。目标已有 `.yss-template.json` 时 fail closed。

第一版只做研发管理资产实例化：不生成前后端运行时、不登记实现仓库、不替代 Harness Orchestrator。

使用方法和跨仓库契约以模板仓为准：

- [create-yss-harness 外部 CLI 实践指南](https://github.com/iloveZzz/yss-harness-dev-agent/blob/main/docs/user-guide/外部命令行工具实践指南.md)
