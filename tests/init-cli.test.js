...
test("interactive init generates a template instance in an empty directory", () => {
  ...
  assert.ok(fs.existsSync(path.join(targetDir, "docs/process/harness-profile.yaml")));
});
...
test("attach and init reject create-yss-spec family metadata", () => {
  ...
  fs.mkdirSync(targetDir, { recursive: true });
  fs.writeFileSync(
    path.join(targetDir, ".yss-template.json"),
    JSON.stringify({ templateName: "create-yss-spec" }, null, 2),
    "utf8",
  );
  fs.writeFileSync(path.join(targetDir, "keep-me.txt"), "runtime\n", "utf8");
  ...
  assert.match(initResult.stderr, /create-yss-spec/);
  assert.equal(fs.readFileSync(path.join(targetDir, "keep-me.txt"), "utf8"), "runtime\n");
