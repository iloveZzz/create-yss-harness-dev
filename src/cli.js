...
function renderTemplateFile(relativePath, content, variables) {
  ...
  if (relativePath === "README.md") {
    let renderedContent = content
      .replace(/^# YSS Harness Agent 模板源/m, `# ${variables.projectName}`)
      .replace(
        /^本仓库是 Harness Agent 的 `template-source`。/m,
        `本仓库是 ${variables.projectName} 的开发落地 Harness 实例（\`project-instance\`）。`,
      )
      .replace(
        /^> 四角色、五阶段 Harness/m,
...
function loadTemplateMetadata(targetDir) {
  assertNotForeignFamily(targetDir);
  const metadataPath = targetPath(targetDir, TEMPLATE_METADATA_FILENAME);
